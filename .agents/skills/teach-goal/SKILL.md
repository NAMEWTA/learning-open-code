...
扫描仓库结构（顶层目录、README、关键配置文件、技术栈），派发 teach subagent 生成 L0 项目总览主题：`teach/<project>/00-overview/lessons/0001-project-map.html` + `teach/<project>/00-overview/reference/00-overview.html`。**派发 prompt 必须要求 subagent 首先 Read 并激活 `.agents/skills/teach/SKILL.md`**。L0 是所有后续 goal 的锚点，必须最先完成、通过 `audit_topic.py` 并通过审查。内容标准见 [references/five-levels.md](references/five-levels.md)。
...
| 层级 | teach 主题目录 | 产出文件 |
|------|---------------|---------|
| L0 项目总览 | `00-overview/` | `lessons/0001-project-map.html` + `reference/00-overview.html` |
| L1 模块总览 | `module-<slug>/` | `lessons/0001-<slug>-module-tour.html` + `reference/<slug>-overview.html` |
| L2 垂直切片 | `slice-<slug>/` | `lessons/0001-*.html` 起；复杂链路拆多节短课 |
| L3 微观 API | `module-<slug>/`（与 L1 同目录） | `reference/<slug>-api.html`；模块主题必须已有 lesson |
| L4 深度剖析 | `deep-dive-<slug>/` | `lessons/0001-*.html` 起；复杂主题拆多节短课 |

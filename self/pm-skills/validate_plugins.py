...
插件集合验证器
================
根据 Claude Code 插件规范验证集合中的所有插件：
- plugin.json 清单：必填字段、作者署名、关键词
- Skills：YAML frontmatter（name 必须与目录匹配，description 必需）
- Commands：YAML frontmatter（description 和 argument-hint 必需）
- 交叉引用：commands 引用了同一插件中实际存在的 skills
- README：存在且包含预期章节

参考依据：
- Anthropic plugin-dev README (https://github.com/anthropics/claude-code/tree/main/plugins/plugin-dev)
- agentskills.io 规范
- Claude Code plugins 参考文档 (https://code.claude.com/docs/en/plugins-reference)
...
def parse_yaml_frontmatter(content: str) -> Optional[dict]:
    """从 markdown 文件中提取 YAML frontmatter（位于 --- 标记之间）。"""
...
def count_words(content: str) -> int:
    """统计 markdown 内容中的单词数（排除 frontmatter）。"""
    # 去除 frontmatter
...
def validate_cross_references(plugin_dir: str, skill_names: list[str]) -> ValidationResult:
    """检查 commands 引用的 skills 是否实际存在于本插件中。"""
    result = ValidationResult()

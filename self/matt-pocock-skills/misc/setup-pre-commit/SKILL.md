...
description: 在当前仓库中设置 Husky pre-commit 钩子，包含 lint-staged（Prettier）、类型检查和测试。适用于用户想要添加 pre-commit 钩子、设置 Husky、配置 lint-staged 或添加提交时格式化/类型检查/测试的场景。
---
...
检查 `package-lock.json`（npm）、`pnpm-lock.yaml`（pnpm）、`yarn.lock`（yarn）、`bun.lockb`（bun）。使用当前存在的那个。如果不确定，默认使用 npm。
...
这将通过新的 pre-commit 钩子运行——这是一个很好的冒烟测试，验证一切正常。

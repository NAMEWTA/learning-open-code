...
**扫描前先划定范围 — YAGNI。** 深化一个模块的价值在于让未来的变更更容易，因此要特别关注代码库中近期发生过变更的部分。先决定*看哪里*，再去看：

- 如果用户指明了方向——某个模块、某个子系统、某个痛点——直接采用它，跳过下面的推断。
- 否则，往回翻阅一段足够长的提交历史（`git log --oneline`），找出代码库的热点区域——反复出现的文件和位置——让这些路径首先牵引你的注意力。如果变更散落各处、没有明确热点，就扩大搜索范围。
...
一旦用户选择了候选方案，运行 `/grilling` 技能与他们一起遍历决策树 — 约束、依赖、深化模块的形状、缝合点后面的内容、哪些测试存留下来。

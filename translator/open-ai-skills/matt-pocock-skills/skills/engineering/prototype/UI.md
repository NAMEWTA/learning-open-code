...
一旦有变体胜出，先捕获答案——是哪一个以及为什么——然后按照 [SKILL](SKILL.md) 描述的方式捕获原型。把胜出者整合进真实代码，把其余部分移到临时分支上，而不是 main：

- **子形式 A** — 将胜出者融合到现有页面中；把落选的变体和切换器从 main 中移除。
- **子形式 B** — 将胜出的变体提升为真正的路由；把临时路由和切换器从 main 中移除。

完整的变体集合是一手资料，所以它落在临时分支上，而不是垃圾桶——留在 main 分支中的变体组件和切换器腐烂很快，会困惑下一个阅读者。
